...
export const LineChart = ({ data, title, color }: {
            ...
            <div className="bg-white rounded-2xl shadow-xl p-6 hover:shadow-2xl transition-all duration-500 transform hover:-translate-y-1">
                <div className="flex items-center justify-between mb-6">
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <LabelShadcn
                            text="admin:error.error-loading-data" translate className="text-gray-500 text-sm" />
                    </div>
                </div>
    ...
    return (
        <div className="bg-white rounded-2xl shadow-xl p-6 hover:shadow-2xl transition-all duration-500 transform hover:-translate-y-1">
            <div className="flex items-center justify-between mb-6">
                ...
                <div className="flex items-center gap-2">
                    <div className={`w-3 h-3 rounded-full ${color}`} />
                    <LabelShadcn text={`${data.length} admin:chart.point-data`} translate className="text-sm text-gray-500 font-medium" splitAndTranslate />
                </div>
            </div>
            ...
            <div className="relative h-64 bg-gradient-to-t from-gray-50/50 to-white rounded-lg p-4">
                ...
                <div className="relative h-full">
                    ...
                    {data.map((item, index) => {
                        ...
                        return (
                            ...
                            >
                                {/* Data point */}
                                <div className={`w-2 h-2 ${color} rounded-full border-2 border-white shadow-lg group-hover:scale-150 transition-transform duration-200`} />
                ...
                <div className="absolute -left-8 top-2 h-full flex flex-col justify-between text-xs text-gray-500 font-medium pb-4">
                    <span className="bg-white px-2 py-1 rounded shadow-md border border-gray-200">{maxValue.toLocaleString()}</span>
                    <span className="bg-white px-2 py-1 rounded shadow-md border border-gray-200">{Math.round(maxValue * 0.75).toLocaleString()}</span>
                    <span className="bg-white px-2 py-1 rounded shadow-md border border-gray-200">{Math.round(maxValue * 0.5).toLocaleString()}</span>
                    <span className="bg-white px-2 py-1 rounded shadow-md border border-gray-200">{Math.round(maxValue * 0.25).toLocaleString()}</span>
                    <span className="bg-white px-2 py-1 rounded shadow-md border border-gray-200">0</span>
                </div>

                {/* X-axis labels */}
                <div className="mt-2 flex justify-between text-xs text-gray-500">
                    <LabelShadcn text={new Date(data[0].date).toLocaleDateString("vi-VN", { month: "short", day: "numeric" })} translate className="text-xs text-gray-500 font-medium bg-white px-2 py-1 rounded shadow-md border border-gray-200" />
                    {data.length > 1 && (
                        <LabelShadcn text={new Date(data[data.length - 1].date).toLocaleDateString("vi-VN", { month: "short", day: "numeric" })} translate className="text-xs text-gray-500 font-medium bg-white px-2 py-1 rounded shadow-md border border-gray-200" />
                    )}
                </div>
            </div>
        </div>
