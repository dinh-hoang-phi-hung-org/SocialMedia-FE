...
import ReportDataTablePagination from "./ReportDataTablePagination";
import { Skeleton } from "@/shared/components/ui/skeleton";
import LabelShadcn from "@/shared/components/ui/LabelShadcn";
import ReportDataTableToolbar from "./ReportDataToolbar";
...
import DetailReportModal from "@/shared/components/BaseLayouts/Modal/DetailReportModal";
...
export default function DataTable<TData, TValue>({
  ...
  const [isDetailReportModalOpen, setIsDetailReportModalOpen] = React.useState(false);
  ...
  const sortingElement = (columnName: string) => {
    return (
      <div>
        {sort.key === columnName && sort.direction === "desc" ? (
          <ArrowDown size={16} />
        ) : sort.key === columnName && sort.direction === "asc" ? (
          <ArrowUp size={16} />
        ) : (
          <ArrowDownUp size={16} />
        )}
  ...
  return (
    <div className="space-y-6">
      <ReportDataTableToolbar
        filter={filter}
        setFilter={setFilter}
        statusTypes={["pending", "banned", "closed"]}
        table={table}
            ...
            {table.getHeaderGroups().map((headerGroup) => (
              <TableRow key={headerGroup.id} className="border-b hover:bg-transparent">
                {headerGroup.headers.map((header) => {
                  if (header.id === "orderNumber" || header.id === "status" || header.id === "reporter")
                    return (
                      <TableHead
                        key={header.id}
                      ...
                      >
                        <LabelShadcn
                          translate
                          text={`report-management:fields.${changeTextCasesToAnother({ input: header.id, fromCase: "camelCase", toCase: "kebab-case" })}`}
                          inheritedClass
            ...
            {loading ? (
              ...
              table.getRowModel().rows.map((row, index) => (
                <TableRow
                  ...
                  onClick={() => {
                    setReportUuid((row.original as TReport).uuid || "");
