...
// import { CreateUserFormDialog } from "../forms/CreateUserFormDialog";
...
const List = <T extends DefaultItemType>(props: ListProps<T>) => {
  ...
  const getListAPI = TypeTransfer[props.typeString]?.otherAPIs?.getUsers;

  // const handleToggleActive = async (userId: string, newIsActive: boolean) => {
  //   try {
  //     // Optimistically update the local state
  //     setListItems((prevItems) =>
  //       prevItems.map((item) => (item.uuid === userId ? ({ ...item, isActive: newIsActive } as T) : item)),
  //     );
  //     if (newIsActive) {
  //       // await TypeTransfer[props.typeString].updateAPI?.(userId, { isActive: true });
  //     } else {
  //       // await TypeTransfer[props.typeString].updateAPI?.(userId, { isActive: false });
  //     }
  //     listFetch();
  ...
  //     //     description: "user-management:notification.error.update",
  //     //   });
  //   }
  // };
      ...
      <div className="flex items-center justify-between space-y-2">
        <div className="flex items-center gap-5">
          ...
          <div className="flex flex-col">
            ...
            />{" "}
            <LabelShadcn translate className="text-muted-foreground" text="user-management:title.description" />
          </div>
        </div>
        {/* <div className="flex items-center space-x-2">
          <Button variant="outline" onClick={() => setIsCreateDialogOpen(true)}>
            <LabelShadcn className="text-inherit" text="user-management:button.create" translate />
          </Button>
        </div> */}
      </div>
      {/* <CreateUserFormDialog isOpen={isCreateDialogOpen} onClose={() => setIsCreateDialogOpen(false)} /> */}
      <UserDataTable<T, unknown>
        loading={loading}
